...
const port = 5000;
const database = 'mongodb+srv://user:user@cluster0.qragp.mongodb.net/dbname?retryWrites=true&w=majority';
const app = express();
...
app.use(express.static(path.join(__dirname,'front')));
 ...
 app.get('/getChatDetails',(req, res)=> {
      mongoClient.connect(database, async (err,db) => {
            if(err)
                throw err;
            else {
        var currentCollection = db.db(dbname).collection(chatCollection);
        currentCollection.find({},{projection: {_id:0}}).toArray((err,result) => {
                    if(err)
                        throw err;
                    else {
                        //console.log(res);
